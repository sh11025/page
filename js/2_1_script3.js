document.addEventListener('DOMContentLoaded', () => {
  // 세 번째 JSON 파일(data3.json)을 불러옵니다[cite: 3].
  fetch('json/2_1_data3.json')
    .then(response => {
      if (!response.ok) {
        throw new Error('세 번째 데이터 네트워크 응답에 문제가 있습니다.');
      }
    ...
    .catch(error => {
      console.error('데이터3 로딩 에러:', error);
    });
});

function renderSection3(marketData) {
  // 'app-3' 도화지를 타겟으로 잡습니다[cite: 3].
  const appDiv = document.getElementById('app-3');
  let htmlContent = '';
